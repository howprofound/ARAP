...
void Game::DrawMenu()
{
	...
	while (true)
	{
		...
		isEvent = SDL_PollEvent(&event);
		if (event.type == SDL_QUIT) // wci�ni�cie krzy�yka ko�czy apk�
		{
			this->setStatus(true);
			break;
		}
		...
		if (menuIndex == 0)
		{
			...
			{
				if (event.key.keysym.sym == SDLK_DOWN) // w d��
				{
					menuPosition++;
					menuPosition %= 2;
				}
				else if (event.key.keysym.sym == SDLK_UP) // w g�r�
				{
					menuPosition--;
					if (menuPosition < 0)
					{
		...
		else if (menuIndex == 2)
		{
			...
			if (client != NULL && client->connected && client->RecievePlayers()) {
				NumberOfPlayers = client->iResult;
				break;
			}
...
void Game::Play()
{
	while (!this->getStatus())
	{
		...
		{
			DrawFish(players[i]->fish); // rysuje rybk�
		}
		isEvent = SDL_PollEvent(&event);
